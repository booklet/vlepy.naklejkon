                    ...
                    if (!preg_match(SHARED_REGEXP, $page_res['body'], $page_data_matches)) {
                        ...
                        if (!$page_data || empty($page_data['entry_data']['ProfilePage'][0]['graphql']['user'])) {
                            ...
                            if ($user_data['is_private']) {
                                ...
                                $variables = ['id' => $user_data['id'], 'first' => 50];
...
function user_media_recent_query_recursive($cache_key, $page_data, $user_data, $variables, $limit, $cursor = 0, $formatted_data = [])
{
                ...
                case 200:
                    ...
                    if (!preg_match(SHARED_REGEXP, $page_res['body'], $page_data_matches)) {
                        ...
                        if (!$page_data) {
                            ...
                            $variables = ['tag_name' => $tag, 'first' => 50];
...
function tag_media_recent_query_recursive($cache_key, $page_data, $variables, $limit, $cursor = 0, $formatted_data = [])
{
                ...
                case 200:
                    ...
                    if (!preg_match(SHARED_REGEXP, $page_res['body'], $page_data_matches)) {
                        ...
                        if (!$page_data) {
                            ...
                            $variables = ['id' => $location_id, 'first' => 50];
...
function location_media_recent_query_recursive($cache_key, $page_data, $variables, $limit, $cursor = 0, $formatted_data = [])
{
...
function query_client_request($page_data, $variables, $query_hash)
{
    ...
    return client_request('get', '/graphql/query/', [
        'query' => [
...
function run($path, $routes)
{
    ...
    if (!$handler_name) {
        ...
        serve_not_found();
    } elseif (!function_exists($handler_name)) {
...
function storage_set($key, $value, $format = false, $merge = false)
{
    ...
    fputcsv($record_fref, [$key, time(), base64_encode(json_encode($value))], ';');
    fclose($record_fref);
...
function instagram_format_media($raw_data, $external = null)
{
    ...
    if (!empty($raw_data['display_resources'])) {
        $formatted_item['images']['thumbnail'] = [];
        $formatted_item['images']['low_resolution'] = [];
        ...
        foreach ($raw_data['display_resources'] as $thumbnail) {
            switch ($thumbnail['config_width']) {
                ...
                case 320:
                    ...
                    break;
                case 640:
                    $formatted_item['images']['standard_resolution'] = [
                        'url' => $thumbnail['src'],
                        'width' => $thumbnail['config_width'],
                        'height' => $thumbnail['config_height'],
                    ];
...
function log_write()
{
    ...
    foreach ($logs as $row) {
        $raw_logs[] = '[' . @date('d.m.Y H:i', $row['time']) . ', ' . $request_id . '] ' . $row['type'] . ': ' . str_replace(["\r", "\n"], '', $row['text']);
    }
